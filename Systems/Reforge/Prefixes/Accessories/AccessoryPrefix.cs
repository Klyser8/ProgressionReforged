    ...
    : LeveledPrefix(level, chainKey), IAccessoryPrefixProvider
{
    ...
    public override IEnumerable<TooltipLine> GetTooltipLines(Item item)
    {
        ...
        if (Math.Abs(CritDamageMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixCritDamageAcc", CritDamageTooltipAcc.Format((int)((CritDamageMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = CritDamageMult < 1f };
        if (Math.Abs(JumpHeightMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixJumpHeight", JumpHeightTooltip.Format((int)((JumpHeightMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = JumpHeightMult < 1f };
        if (Math.Abs(KnockbackMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixKnockbackResist", KnockbackResistTooltip.Format((int)((KnockbackMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = KnockbackMult < 1f };
        if (Math.Abs(DamageMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixAccessoryDamage", DamageTooltipAcc.Format((int)((DamageMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = DamageMult < 1f };
        if (Math.Abs(ManaRegenMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixManaRegen", ManaRegenTooltip.Format((int)((ManaRegenMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = ManaRegenMult < 1f };
        if (Math.Abs(MovementSpeedMult - 1f) > 0.01f)
            yield return new TooltipLine(Mod, "PrefixMovementSpeed", MovementSpeedTooltip.Format((int)((MovementSpeedMult - 1f) * 100f))) { IsModifier = true, IsModifierBad = MovementSpeedMult < 1f };
    }
}